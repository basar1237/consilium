...
import type { FloatingSocialBlock } from '@/payload-types'
...
export const FloatingSocialBlockComponent: React.FC<FloatingSocialBlock> = ({
  links,
  enableShare,
}) => {
      ...
      <div className="flex flex-col gap-0.5 overflow-hidden rounded-xl bg-[#1A1A2E] shadow-xl">
        {items.map(({ label, icon, href }) => (
          <a
            ...
            aria-label={label}
          >
            {icon && socialIcons[icon] ? socialIcons[icon] : icon}
          </a>
        ))}
        {enableShare && (
          <button
